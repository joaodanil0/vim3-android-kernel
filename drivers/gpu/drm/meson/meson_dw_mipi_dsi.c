...
static int meson_dw_mipi_dsi_probe(struct platform_device *pdev)
{
	...
	struct resource *res;
	int ret;
	pr_crit("meson_dw_mipi_dsi : Teste 1");
	mipi_dsi = devm_kzalloc(&pdev->dev, sizeof(*mipi_dsi), GFP_KERNEL);
	if (!mipi_dsi)
		return -ENOMEM;
	pr_crit("meson_dw_mipi_dsi : Teste 2");
	...
	pr_crit("meson_dw_mipi_dsi : Teste 3");
	...
	pr_crit("meson_dw_mipi_dsi : Teste 4");
	...
	pr_crit("meson_dw_mipi_dsi : Teste 5");
	/*
	...
	pr_crit("meson_dw_mipi_dsi : Teste 6");
	ret = clk_prepare_enable(mipi_dsi->px_clk);
	if (ret) {
		dev_err(&pdev->dev, "Unable to prepare/enable PX clock\n");
		return ret;
	}
	pr_crit("meson_dw_mipi_dsi : Teste 7"); 
	...
	pr_crit("meson_dw_mipi_dsi : Teste 8");
	...
	pr_crit("meson_dw_mipi_dsi : Teste 9");
